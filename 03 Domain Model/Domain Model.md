...
제품의 공식 장기 목표는 Desktop App이며, 첫 Web UI는 Desktop App Shell에 재사용할 수 있는 Desktop-ready UI로 구현한다.

## UI Shell

Web UI를 실행하고 사용자 입력과 서버 결과를 표시하는 제품 표면. 초기 UI Shell은 브라우저이고 장기적으로 Desktop App Shell을 사용한다. UI Shell은 SQLite, Git이나 파일시스템을 직접 변경하지 않는다.

## Local Transport

UI Shell과 App-managed Local Runtime의 application service를 연결하는 통신 경계. MVP는 localhost HTTP API를 우선하며, 향후 Desktop App에서는 같은 HTTP API 또는 local IPC를 사용할 수 있다. Local Transport는 Internal Tool Contract 자체가 아니다.
...
`Node`는 내부 시스템 용어다. 사용자 화면에서는 Personal Server를 별도 서버 제품처럼 노출하지 않고 `앱`, `Local Runtime`, `연결된 Device`, `이 컴퓨터` 같은 표현을 사용한다.
...
내부 배치 용어로 유지할 수 있지만 제품 UX에서는 별도 서버 제품이 아니라 앱이 관리하는 Local Runtime으로 설명한다.

## Personal Runtime

Desktop App 또는 초기 Web UI가 시작하고 관리하는 로컬 실행 기반. Local Control Plane, Owner Runtime, Agent Run Engine, Worker Supervisor, SQLite, Git Worktree와 Artifact Store를 실행한다. 장기적으로 Background Service로 분리할 수 있지만 설치 방식은 아직 확정하지 않는다.
...
개인 모드 MVP에서는 사용자 가입을 구현하지 않고 첫 설치 시 local user 한 명을 자동 생성한다. 그래도 향후 중앙 account 연결과 다중 사용자 기능을 위해 user 엔티티, user_id 참조, Device와 Session, 프로젝트 소유권과 권한 구조를 유지한다.

사용자 정보를 전역 상수나 하드코딩된 단일 ID로 처리하지 않는다.

## Local User

Local Runtime이 자동 생성하고 로컬 프로젝트 소유권, Owner 권한 위임, Device와 Session을 연결하는 Human 사용자 기록. 중앙 account와 별개의 Identity다.

## Future Account와 Account Link

향후 다중 기기 동기화, 라이선스·구독과 팀 Membership에 사용하는 중앙 account와 Local User의 nullable 연결. `account_id`와 `account_link_status: local_only | linked | unlinked | revoked`를 표현할 수 있어야 한다. MVP는 중앙 로그인이나 동기화를 구현하지 않는다.
...
신뢰되거나 등록된 앱, 브라우저 또는 Node 단위. `desktop_app`, `web_ui`, `local_runtime`, `worker_node`, `test_runner_node`, `unknown` type 후보를 가진다. Worker Node와 Test Runner Node의 상세 모델은 ADR-0012에서 다룬다.

## Session

특정 Device에서 현재 인증된 접속 상태. `device_id`, `local_user_id`, nullable `account_id`, `token_hash`, 생성·마지막 접속·idle/absolute 만료·폐기 시각을 추적한다. Device 폐기, 특정 Session 폐기와 모든 Session 로그아웃을 지원한다.

## Pairing Code

Web UI를 Local Runtime에 연결해 Device와 Session을 발급하기 위한 짧은 수명의 1회용 인증 정보. MVP 권장값은 10분 만료와 1회 사용이며 원문 대신 hash를 저장한다. 원격 접속 기능과는 별개다.

## Token Hash

Opaque random Session token의 검증용 hash. SQLite에는 token 원문을 저장하지 않는다. Web UI는 HttpOnly, SameSite cookie를 권장하고 localStorage에 민감 token을 저장하지 않는다.

## 다중 기기 동기화 경계

같은 account의 여러 Device 동기화는 장기 목표지만 MVP에서는 각 Device가 독립 Local Runtime을 가진다. `account_id`, `device_id`와 sync-ready metadata만 열어 두며 Conversation, Task, Agent Run, Artifact와 Git 상태 동기화는 후속 설계다.

Remote Test Runner는 Worker Capability로 사용하며 사용자 Device나 AI Worker와 구분한다. 등록, 실행과 artifact 경계는 [[07 ADR/ADR-0012 Remote Test Runner Worker Capability]]를 따른다.
...
외부 AI CLI process를 Owner 또는 Worker 도메인 역할에 연결하는 교체 가능한 실행 구현체. Adapter는 Human, Owner, Worker 또는 Node Identity가 아니다. 플랫폼은 외부 CLI 내부 state를 소유하지 않고 prompt/context, working directory, environment, timeout, stdout/stderr, exit status, artifact와 Git 결과를 관찰·기록한다.

## Codex CLI Owner Adapter

1차 MVP에서 Owner Agent Run을 수행하는 목표 외부 process Adapter. Codex 자체가 Owner Identity인 것은 아니다.

## Antigravity CLI Worker Adapter

1차 MVP에서 Task Attempt를 수행하는 목표 외부 process Adapter. Antigravity 자체가 Worker Identity인 것은 아니다.

## Mock Adapter

실제 외부 AI CLI 없이 deterministic 결과나 작은 변경을 만드는 개발·pipeline test Adapter. 제품의 목표 Adapter를 대체하지 않는다.

## Manual Adapter

플랫폼이 Task Attempt와 Worktree를 준비하고 Human이 해당 Worktree에서 작업한 결과를 제출하도록 하는 개발·복구용 임시 Adapter. Human과 Worker Identity를 합치는 것이 아니며 자동화 목표를 대체하지 않는다.
...
전체 MVP 중 독립적으로 검증 가능한 첫 구현 범위. 1차 Slice는 Mock/Manual Adapter로 Task, Attempt, Worktree, commit, Owner review, 사용자 승인과 squash merge를 관통한다.

## Golden Path

Implementation Slice의 end-to-end 완료 기준. Golden Path 1은 README 한 줄 수정, Golden Path 2는 작은 함수 수정과 local test, Golden Path 3은 Personal Alpha의 작은 게임 제작 검증이다. Mock/Manual Adapter는 앞선 Golden Path 개발을 가능하게 하지만 목표 Codex/Antigravity Adapter 방향을 대체하지 않는다.
...
모든 Tool Call은 Tool 이름과 버전, caller, 관련 Run/Project/Task 식별자, risk level, idempotency key, correlation ID, arguments, status, 결과 또는 오류와 시간을 담는 공통 Envelope를 가진다.

## Internal Tool Contract

Owner Agent Run이 Local Control Plane의 application service를 호출하는 서버 내부 계약. HTTP endpoint와 1:1로 고정하지 않으며 내부 호출, HTTP API, 향후 IPC와 background queue가 같은 업무 규칙을 재사용하게 한다.

## Tool Registry

플랫폼이 소유한 상태와 부작용을 통제하는 Tool Definition의 버전 가능한 Registry. 각 정의는 input/output schema 참조, 부작용 여부, 기본 위험도, 필요한 Grant, scope evaluator, idempotency·승인·감사 요구, 활성화와 deprecation 정보를 가진다. 최종 Tool 목록은 고정하지 않는다.

## Tool Boundary

행동이 플랫폼의 정상 실행 틀 안에 있는지 판단하는 경계. Owner, UI와 Worker는 이 경계를 우회해 SQLite, Git, 파일시스템이나 실행 상태를 직접 변경하지 않는다. Tool Boundary는 행동의 유효성을 판단하고, 권한·승인 판단은 Local Policy Engine에 맡긴다.
...
Approval Interruption은 Tool Call과 Agent Run의 `waiting_for_approval` 상태를 연결한다. 승인 뒤 arguments, repository, base commit, risk level 또는 변경 범위가 달라지면 stale로 처리할 수 있어야 한다.
...
Command Policy는 하드코딩된 세밀한 명령 allowlist보다 Owner Grant, Autonomy Profile과 Project Settings로 사용자의 실행 환경 경계를 표현한다. 기본값은 보수적으로 두되 사용자는 자신이 가진 권한 범위에서 프로젝트, repository, 경로와 risk 범위를 넓힐 수 있다. R4 자동 실행 금지는 유지한다.
...
Personal Mode의 Primary Personal Server에서 Owner Grant, Autonomy Profile, Project Settings, Session/Device 권한, Tool Registry와 Risk Level을 평가하는 Policy Engine. Tool Call을 허용, 승인 대기 또는 거절로 분류한다. Personal Mode에는 이 평가를 위한 Central Authority가 필요하지 않다.
...
Worker가 특정 Task Attempt를 실행할 권리를 획득한 기록. 하나의 active Task Attempt에는 동시에 하나의 유효한 claim만 허용한다.

## Worker Lease

Worker Claim의 유효 기간. Worker heartbeat로 갱신하며 결과 제출, 취소 또는 release로 종료한다. 만료되면 Attempt를 재시도 가능한 상태로 전환할 수 있다.

## ArtifactRef

Artifact Store에 저장된 diff, 테스트·린트·빌드 로그, Worker report, 오류 기록과 기타 산출물을 가리키는 SQLite 참조. Agent Run, Tool Call, Worker Run과 Task Attempt에 연결할 수 있으며 경로, 종류, content type, 크기, checksum과 보존 정책 참조를 가진다.

## Remote Test Runner Node

Worker가 원격 test, build와 lint를 실행하기 위해 사용하는 등록된 실행 Node. Human, Owner, Worker와 다른 Identity이며 독립 AI Worker가 아니다. Project, Repository와 Capability별 허용 범위, network kind, heartbeat와 상태를 가진다.
...
Worker가 유효한 Task Attempt claim 안에서 Remote Test Runner를 호출하는 Worker Tool/Capability. Tool Call Envelope의 caller는 Worker이며 Tool Boundary와 Policy Engine을 통과한다. Owner가 직접 호출하지 않는다.

## Test Run

특정 Task Attempt가 특정 Runner에서 commit SHA와 test target을 실행한 기록. 상태 후보는 `created`, `queued`, `dispatching`, `running`, `uploading_artifacts`, `analyzing`, `succeeded`, `failed`, `cancelled`, `timed_out`, `lost`다. retry 관계, timeout과 `artifact_refs`를 추적한다.

## Worker Report

Worker가 Test Run의 로그와 artifact를 분석해 Owner에게 제출하는 결과. 실행 환경, commit, test target, 명령 요약, 상태, 핵심 로그, failure hypothesis, retry와 `artifact_refs`를 포함한다. Owner와 UI는 `artifact_ref`로 원본 artifact를 열람할 수 있다.

Remote Test Runner checkout은 Git source의 임시 실행 복사본이다. Runner는 코드를 수정하거나 commit, merge, 배포하지 않는다.
